...
if 'all_questions' not in st.session_state:
    raw_data = [
        ("うるさい", ["活気がある", "元気がある", "賑やか", "声が通る"], "活気がある", "「賑やか」や「元気」も近いですが、周囲を巻き込むエネルギーの源泉を肯定する言葉として「活気」が最も適しています。"),
        ("理屈っぽい", ["論理的である", "頭が良い", "説明が丁寧", "こだわりがある"], "論理的である", "「説明が丁寧」も長所ですが、理屈の根底にある「論理（ロジック）」そのものを能力として称える変換です。"),
        ("飽きっぽい", ["好奇心が旺盛", "行動が早い", "流行に敏感", "多趣味"], "好奇心が旺盛", "「行動が早い」ことも事実ですが、飽きる原因である「新しいものへの関心」を肯定するのがこの変換の肝です。"),
        ("頑固", ["自分を持っている", "意思が強い", "真面目", "ぶれない"], "自分を持っている", "「意思が強い」よりも、他者に流されない「個の確立」に焦点を当てることで、より深い敬意を表せます。"),
        ("優柔不斷", ["思慮深い", "慎重である", "人の意見を尊重する", "柔軟である"], "思慮深い", "「慎重」よりも、決断の背景にある「思考の深さ」を評価することで、停滞を肯定的なプロセスに変えます。"),
        ("ケチ", ["経済観念がある", "節約家", "質素", "管理が行き届いている"], "経済観念がある", "単に「使わない（節約）」ことではなく、全体を俯瞰して管理する「理性的判断」を称える表現です。"),
        ("気が短い", ["スピード感がある", "情熱的", "決断が早い", "感受性が豊か"], "スピード感がある", "短気という欠点を、物事を即座に前に進める「推進力」という強みに変換しています。"),
        ("おせっかい", ["面倒見が良い", "社交的", "気が利く", "愛情深い"], "面倒見が良い", "介入を「余計なこと」と切り捨てず、相手を思いやる「行動力」として再定義しています。"),
        ("生意気", ["物怖じしない", "堂々としている", "自信がある", "頼もしい"], "物怖じしない", "生意気に見える原因である「相手に怯まない姿勢」を、勇気として評価する変換です。"),
        ("いい加減", ["大らか", "細かいことにこだわらない", "柔軟性が高い", "適応力がある"], "大らか", "「細かいことにこだわらない」の結果得られる、精神的なゆとりや器の大きさを称える言葉です。"),
    ]
...
if st.session_state.page == "ホーム":
    ...
    if st.session_state.get('show_result'):
        # 間違った際になぜ違うのかを含めた解説を表示
        is_correct = (st.session_state.selected_option == q['answer'])
        header_text = f"正解: {q['answer']}" if is_correct else f"正解は「{q['answer']}」でした"
        
        st.markdown(f"""
            <div class="feedback-box">
                <div style="font-weight: bold;"><span class="feedback-lightbulb">💡</span> {header_text}</div>
                <div class="explanation-text">
                    <b>【なぜこの言葉？】</b><br>{q['explanation']}
                </div>
            </div>
        """, unsafe_allow_html=True)
